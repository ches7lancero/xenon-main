...
class Premium(wkr.Module):
    ...
    async def chatlog(self, ctx):
        """
        Save & load messages from individual channels, you can find more help on the [wiki](https://wiki.xenon.bot/chatlog)
        """
    ...
    async def sync(self, ctx):
        """
        Sync messages and bans between different servers and channels, you can find more help on the [wiki](https://wiki.xenon.bot/sync)
        """
        await ctx.f_send("This command can **only** be used with **Xenon Premium**.", f=ctx.f.INFO)
        await ctx.invoke("premium")
